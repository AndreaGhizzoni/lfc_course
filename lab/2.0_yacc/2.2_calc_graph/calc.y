%{
    ...
    int ex(nodeType* p, int flush);

    int yylex(void);
    void yyerror(const char* s);

    int sym[26]; /* symbol table */
    int uid = 0; // used into dot generation process
%}
...
%type <nPtr> function stmt expr stmt_list
...
program: function {ex($1, 1); freeNode($1); exit(0);}
       ;

function: function stmt {$$=$2; ex($2, 0);}
        | /*empty*/ {}
        ;
